...
% Define the time span and initial condition (modify as needed)
AP_initcond = 0;
B_initcond = 1*10^6;
...
% Vectorized approach: Loop over nu values
for j = 1:length(nu_vals)  % Loop over nu values
    % Get the current value of nu
    nu = nu_vals(j);

    % Loop over the meshgrid, avoiding the nested loop for EL and RL
    for i = 1:numel(EL_matrix)
        % Get corresponding EL and RL for each point in the meshgrid
        EL_initcond = EL_matrix(i)
        RL_initcond = RL_matrix(i)

        % Set the new initial conditions or parameters here
        init_cond = [EL_initcond RL_initcond AP_initcond B_initcond 0]';

        % Run the model using ODE solver
        ...
        T_vals(i, j) = T(end)./365;  % Store the ratio for this combination of EL and nu
    end
end

% After the loop, you can visualize the result as a heatmap
% Plot the heatmap
figure;
