...
% Figure Name
figurename = ['DosingNoDepletionFigures/DosingdiffHeatMapNoDepletion_', disease_type, '_nuis', num2str(nu), '.pdf'];
...
max(T_vals - disease_timing);
% Define custom colormap
figure(1)
imagesc(B_vals, Dose_vals, T_vals - round(disease_timing));
xlim([min(B_vals) max(B_vals)])
ylim([min(Dose_vals) max(Dose_vals)])
xlabel('Beta cells', 'FontSize', 17);
