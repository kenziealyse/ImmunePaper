...
for i = 1:length(nu_vals)
    ...
    % Run the Model
    options = odeset('Events', @(t, Y) PercentBetaCellMassEvent(t, Y, NuRegTcellmodel(t,Y, params, timing), B_initcond));
    [T,Y] = ode23s(@(t,Y) NuRegTcellmodel(t,Y, params, timing), tspan1, init_cond, options);
...
legend('R_L(0) = 10', 'R_L(0) = 1 \times 10^6', 'R_L(0) = 1 \times 10^6, APC timing = 5 years', 'Location', 'best')
