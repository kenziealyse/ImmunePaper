...
AL_initcond = 0;
EL_initconds = 10:1000000:1*10^8;
RL_initcond = 10;
AP_initcond = 0;
EP_initcond = 0;
RP_initcond = 0;
...
T_vals = zeros(length(EL_initconds), 1);
lambda = zeros(length(EL_initconds), 1);

params = [phiA deltaA lambdaEL omegaEL phiE deltaE lambdaR omegaR C ...
          phiR deltaR kappa r1 r2 nu]';


for i = 1:length(EL_initconds)

    EL_initcond = EL_initconds(i);
...
plot(EL_initconds, T_vals, 'LineWidth', 1.3, 'Color', 'k')
xlabel('EL(0)', 'FontSize', 17)
