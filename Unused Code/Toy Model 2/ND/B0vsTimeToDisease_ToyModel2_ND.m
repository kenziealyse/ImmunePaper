...
N = 100;
B_vals = linspace(10,1e6, N); 

figurename = ['B0vsTTD_ToyModel2_ND_', num2str(nu), '.pdf'];

EL_initcond = 10/C;
RL_initcond = 10/C;
AP_initcond = 0;
B_vals = (B_vals.*nu.*lambdaR)/omegaR^2;
...
'hi'
figure(1)
plot(omegaR^2.*B_vals./(nu.*lambdaR), time./omegaR, 'Color', 'k', 'LineWidth', 1.5, 'LineStyle', '-')
hold on
...
figure(1)
plot(omegaR^2.*B_vals./(nu.*lambdaR), time./omegaR, 'Color', 'r', 'LineWidth', 1.5, 'LineStyle', '--')
hold on
% xline(0.01)
ax = gca;
ax.FontSize = 21;
ylabel('Time to 20% Beta Cell Mass')
xlabel('\beta(0)', 'FontSize', 30)
