...
% Figure Name
figurename = ['Increase DeltaA Figures/DosingdiffHeatMapwithDepletiondeltaA_', disease_type, '_nuis', num2str(nu), '.pdf'];
...
for i = 1:length(Dose_Time_vals)
    ...
    for j = 1:length(Dose_vals)
        ...
        % Run the Model
        options = odeset('Events', @(t, Y) PercentBetaCellMassEvent(t, Y, NuRegTcellmodel_increasedeltaA(t,Y, params, Dose_Time), Init_B));
        [T_predose,Y_predose] = ode23s(@(t,Y) NuRegTcellmodel_increasedeltaA(t,Y, params, Dose_Time), tspan1, init_cond, options);
        ...
        % Run the Model
        options = odeset('Events', @(t, Y) PercentBetaCellMassEvent(t, Y, NuRegTcellmodel_increasedeltaA(t,Y, params, Dose_Time), Init_B));
        [T_postdose,Y_postdose] = ode23s(@(t,Y) NuRegTcellmodel_increasedeltaA(t,Y, params, Dose_Time), tspan2, init_cond, options);
