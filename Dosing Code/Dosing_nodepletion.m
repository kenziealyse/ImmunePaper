...
disease_type = 'early';
nu = 1e-5;
end_time = 2*365; % Only dose up to time of disease
...
% Figure Name
figurename = ['DosingNoDepletionFigures/DosingHeatMapNoDepletion_', disease_type, '_nuis', num2str(nu), '.pdf'];
...
xlabel('Dose Times, years', 'FontSize', 17);
...
% Set z axis limits
zlimit = ceil(max(max(T_vals))/10)*10;
if zlimit == 0
    zlimit = 10;
end
clim([min(min(T_vals)), zlimit]);  % Set color axis limits based on data range
