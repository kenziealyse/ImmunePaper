...
for i = 1:length(Dose_Time_vals)
        ...
        % Run the Model
        options = odeset('Events', @(t, Y) PercentBetaCellMassEvent(t, Y, NuRegTcellmodel_withdepletion_v2(t,Y, params, Dose_Time), Init_B));
        [T_predose,Y_predose] = ode23s(@(t,Y) NuRegTcellmodel_withdepletion_v2(t,Y, params, Dose_Time), tspan1, init_cond, options);
        ...
        % Run the Model
        options = odeset('Events', @(t, Y) PercentBetaCellMassEvent(t, Y, NuRegTcellmodel_withdepletion_v2(t,Y, params, Dose_Time), Init_B));
        [T_postdose,Y_postdose] = ode23s(@(t,Y) NuRegTcellmodel_withdepletion_v2(t,Y, params, Dose_Time), tspan2, init_cond, options);
