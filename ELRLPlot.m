...
figurename = ['Figures/ELvsRLplot', num2str(nu), '.pdf'];
...
for i = 1:length(r1_vals)
    ...
    if i == 4
        ...
        hold on
        scatter(EL(end)./scale,RL(end)./scale, 'Color', colors{i}, 'LineWidth', 5, 'Marker', '*') 
...
figure(1)
plot([kE./scale 0], [0 kR./scale], '--', 'Color', "k", 'HandleVisibility', 'off', 'linewidth', 2)
xlim([0 0.1])
