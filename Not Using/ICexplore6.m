...
subplot(2,2,1)
rate1 = lambdaEL.*AL;
rate2 = (omegaEL).*(1 - (EL + RL)/C).*EL - phiE.*EL - deltaE.*EL;
totalELrate = rate1 + rate2;
rate1 = lambdaR.*AL;
rate2 = (omegaR).*(1 - (EL + RL)/C).*RL - phiR.*RL - deltaR.*RL;
totalRLrate = rate1 + rate2;
plot(T./365, totalELrate./scale, 'LineWidth', 2, 'Color', 'b', 'LineStyle', '--')
hold on
plot(T./365, totalRLrate./scale, 'LineWidth', 2, 'Color', 'k', 'LineStyle', '-')
% legend('Total EL Rate', 'Total RL Rate')
set(gca, 'Xscale', 'log')
title('Low RL, low nu')
% ylim([-1 10])
xline(0.0057)
...
figure(3)
subplot(2,2,1)
rate = -(kappa.*EP.*B)./(1+r2.*RP);
rate = phiA.*AP - deltaA.*AL;

plot(T./365, AL./scale, 'LineWidth', 2, 'Color', 'r', 'LineStyle', '-')
set(gca, 'Xscale', 'log')
xline(0.0057)
